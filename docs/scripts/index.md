...
| [`claude-tmux-status`](tmux.md#claude-tmux-status)           | Flag the tmux window red (or set the terminal title) when Claude Code is waiting for input. Driven by Claude Code's `Stop`/`Notification`/`UserPromptSubmit`/`SessionEnd` hooks. |
